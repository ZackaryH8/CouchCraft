...
function sideBase(active: boolean, capsOn?: boolean) {
  if (active)  return "flex w-full flex-col items-center justify-center gap-1.5 rounded-[0.9rem] border border-lime-300/60 bg-lime-300 text-slate-950 font-semibold text-sm transition-colors duration-100";
  if (capsOn)  return "flex w-full flex-col items-center justify-center gap-1.5 rounded-[0.9rem] border border-lime-300/30 bg-lime-300/10 text-lime-300 font-semibold text-sm transition-colors duration-100";
...
function sideRowBase(active: boolean) {
  if (active) return "flex w-full items-center justify-center gap-2 rounded-[0.9rem] border border-lime-300/60 bg-lime-300 text-slate-950 font-semibold text-sm transition-colors duration-100";
...
export function OSK({ value, cursorPos, focus, isShift, flashKey, controllerType }: OSKProps) {
  ...
  const rightActive = (i: number) => (focus.section === "right" && focus.sideIdx === i) || (i === 0 && flashKey === "backspace") || (i === 1 && flashKey === "cur_right");
  const spaceActive = (focus.section === "main" && focus.row === 4) || flashKey === "space";
      ...
      <div className="simple-panel w-fit rounded-[2rem] px-8 py-8">
        ...
        <div className="mb-5 rounded-[1.25rem] border border-white/10 bg-black/30 px-6 py-4">
          ...
          <p className="mt-2 min-h-10 whitespace-pre font-mono text-3xl tracking-tight text-white">
            {value.length === 0 ? (
              <>
                <span className="text-stone-600">Start typing…</span>
        ...
        <div className="flex gap-3">
          ...
          <div className="flex w-32 flex-col gap-2">
            ...
            <div className={`${sideBase(leftActive(0))} ${SPAN2}`}>
              <GamepadGlyph controller={controllerType} button="lb" size={24} />
              <span>← Cursor</span>
            </div>

            {/* Caps: L3 (Xbox) / L2 (PS) */}
            <div className={`${sideBase(leftActive(1), isShift)} ${SPAN2}`}>
              <GamepadGlyph controller={controllerType} button={capsIcon(controllerType)} size={24} />
          ...
          <div className="flex flex-col gap-2">
            {MAIN_ROWS.map((keyRow, ri) => (
              <div key={ri} className="flex gap-2">
                {ri === 4 ? (
                  <div className={`${keyBase(spaceActive)} ${SPACE_W} gap-3 text-sm font-semibold`}>
                    <GamepadGlyph controller={controllerType} button={spaceIcon(controllerType)} size={22} />
                    <span>Space</span>
                  </div>
                ) : (
                  keyRow.map((key, ci) => (
                    <div key={key} className={`${keyBase(mainActive(ri, ci))} w-16`}>
                      <span className="text-lg font-semibold">
                        {isShift && /^[a-z]$/.test(key) ? key.toUpperCase() : key}
                      </span>
                    </div>
                  ))
                )}
          ...
          <div className="flex w-32 flex-col gap-2">
            ...
            <div className={`${sideRowBase(rightActive(0))} h-14`}>
              <GamepadGlyph controller={controllerType} button={backspaceIcon(controllerType)} size={22} />
              <span>Backspace</span>
            </div>

            {/* → Cursor: RB / R1 */}
            <div className={`${sideBase(rightActive(1))} ${SPAN2}`}>
              <GamepadGlyph controller={controllerType} button="rb" size={24} />
              <span>Cursor Right</span>
            </div>

            {/* Enter: Menu (Xbox) / R2 (PS) */}
            <div className={`${sideBase(rightActive(2))} ${SPAN2}`}>
              <GamepadGlyph controller={controllerType} button={enterIcon(controllerType)} size={24} />
              <span>Enter</span>
            </div>
          </div>
