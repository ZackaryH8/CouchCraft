    ...
    return (
        <aside className="simple-panel flex w-[19rem] shrink-0 flex-col overflow-y-auto rounded-[2rem] px-6 py-7">
            <div className="mb-10 flex items-center justify-between">
                <div>
                    <p className="font-display text-4xl leading-none tracking-[-0.08em] text-lime-300">
                        CC
                    </p>
            ...
            <nav className="space-y-3">
                {SIDEBAR_ITEMS.map((item, i) => {
                    const isActive =
                        sidebarIndex === i && globalFocus === "sidebar";
                    return (
                        <div
                            key={item.label}
                            ref={(node) => {
            ...
            <div className="mt-auto rounded-[1.6rem] border border-white/8 bg-white/[0.03] p-5">
                ...
                <p className="mt-2 text-sm leading-6 text-stone-400">
                    Controller navigation is active. Instances, updates, and
