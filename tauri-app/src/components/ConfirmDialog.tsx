  ...
  return (
    <div className="absolute inset-0 z-10 flex items-center justify-center rounded-[2rem] bg-black/70 backdrop-blur-sm">
      <div className="simple-panel w-[34rem] rounded-[2rem] px-10 py-9">
        <p className={`text-xs font-semibold uppercase tracking-[0.35em] ${danger ? "text-red-400/80" : "text-lime-300/80"}`}>
          Confirm
        </p>
        <h2 className="mt-3 font-display text-4xl tracking-[-0.05em] text-white">{title}</h2>
        <p className="mt-3 text-lg leading-7 text-stone-400">{message}</p>
        ...
        <p className="mt-4 text-sm text-stone-500">
          Left/Right to choose · A to {choice === 1 ? confirmLabel.toLowerCase() : "cancel"} · B to go back
        </p>
      </div>
    </div>
  );
