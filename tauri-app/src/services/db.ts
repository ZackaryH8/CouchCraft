...
export async function touchLastPlayed(id: string): Promise<void> {
    const db = await getDb();
    await db.execute("UPDATE instances SET last_played_at = $2 WHERE id = $1", [
        id,
...
export async function setContentEnabled(
    ...
    const db = await getDb();
    await db.execute("UPDATE content SET enabled = $2 WHERE id = $1", [
        id,
        enabled ? 1 : 0,
    ]);
}

export async function setContentUpdateAvailable(
    id: string,
    versionId: string,
): Promise<void> {
    const db = await getDb();
    ...
    await db.execute(
        "UPDATE content SET modrinth_version_id = $2 WHERE id = $1 AND update_available = 1",
        [id, versionId],
    );
}
...
export async function loadJavaRuntimes(): Promise<JavaRuntime[]> {
    const db = await getDb();
    const rows = await db.select<JavaRuntimeRow[]>(
        "SELECT * FROM java_runtimes ORDER BY version ASC",
    );
    return rows.map((row) => ({
...
export async function setActiveAccountId(id: string | null): Promise<void> {
    ...
    if (id === null) {
        await db.execute(
            "DELETE FROM settings WHERE key = 'auth.active_account_id'",
            [],
        );
    } else {
...
export async function migrateLegacyAuth(): Promise<void> {
    ...
    const [refreshToken, accessToken, username, uuid, expiresAt] =
        await Promise.all([
    ...
    await db.execute(
        "DELETE FROM settings WHERE key LIKE 'auth.%' AND key != 'auth.active_account_id'",
        [],
    );
}
