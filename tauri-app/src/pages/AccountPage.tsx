...
export function useAccountPage({
  ...
  useEffect(() => { setActionIndex(0); }, [focusedIndex]);
    ...
    (input: GamepadInput) => {
      ...
      switch (input) {
        ...
        case "DOWN":
          ...
          break;
        case "LEFT":
          if (focusedItem?.type === "account") setActionIndex((i) => Math.max(i - 1, 0));
          else toSidebar();
          break;
        case "RIGHT":
          if (focusedItem?.type === "account") setActionIndex((i) => Math.min(i + 1, 1));
          break;
        case "B":
          toSidebar();
          break;
        case "A":
          if (focusedItem?.type === "add") {
            ...
            if (actionIndex === 0) void switchAccount(focusedItem.account.id);
            else void removeAccount(focusedItem.account.id);
          }
          break;
      }
    },
    [signingIn, focusedItem, focusedIndex, actionIndex, items.length,
     cancelSignIn, startSignIn, switchAccount, removeAccount, toSidebar],
  );

  return { items, focusedIndex, setFocusedIndex, actionIndex, setActionIndex, handleInput };
}
          ...
          <div className="flex min-w-0 flex-1 flex-col gap-6 justify-center">
            ...
            <button
              ...
              className={`rounded-[1.25rem] border px-6 py-4 text-center transition duration-200 ${
                hasFocus ? "border-white/15 bg-white/[0.04] text-stone-300" : "border-white/8 bg-transparent text-stone-500"
              }`}
            >
  ...
  return (
    <section className="grid min-h-0 flex-1 grid-cols-[26rem_1fr] gap-6">
      ...
      <div className="simple-panel flex min-h-0 flex-col rounded-[2rem] px-4 py-5 gap-2 overflow-y-auto">
        ...
        {items.map((item, i) => {
            ...
            return (
              <div
                ...
                className={`flex cursor-default items-center gap-3 rounded-[1.25rem] border px-4 py-3 transition duration-200 ${
                  isFocused ? "border-lime-300/60 bg-lime-300/10" : "border-dashed border-white/10 bg-transparent"
                }`}
              >
                <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-[0.75rem] border text-xl ${
                  isFocused ? "border-lime-300/40 text-lime-300" : "border-white/10 text-stone-500"
                }`}>
                  +
                </div>
                <p className={`text-base font-semibold ${isFocused ? "text-lime-300" : "text-stone-500"}`}>
                  Add Account
                </p>
              </div>
            );
          ...
          return (
            <div
              ...
              onMouseEnter={() => onMoveFocus(i)}
              className={`flex cursor-default items-center gap-3 rounded-[1.25rem] border px-4 py-3 transition duration-200 ${
                isFocused ? "border-lime-300/60 bg-[#171c1a]" : "border-white/8 bg-[#121514]"
              }`}
            >
              ...
              <div className="min-w-0 flex-1">
                <p className="truncate text-base font-semibold text-white">{account.mcUsername}</p>
                {isActive && (
                  <p className="text-xs text-lime-300">Active</p>
                )}
              </div>
            </div>
          );
        ...
        {authError && (
          <p className="mt-2 px-2 text-sm text-red-400">{authError}</p>
        )}
      </div>
      ...
      <div className="flex flex-col gap-4">
        {focusedItem?.type === "account" ? (() => {
          ...
          const actions = [
            {
              label: isActive ? "Active Account" : "Set as Active",
              description: isActive ? "This account is currently active" : "Use this account when launching",
              danger: false,
              disabled: isActive,
            },
            {
              label: "Remove",
              description: "Sign this account out of CouchCraft",
              danger: true,
              disabled: false,
            },
          ];
            ...
            <>
              ...
              <div className="simple-panel flex flex-col rounded-[2rem] px-6 py-6">
                ...
                <div className="mt-5 grid grid-cols-2 gap-3">
                  <div className="rounded-[1rem] border border-white/8 bg-white/[0.03] px-4 py-3">
                    <p className="text-xs font-semibold uppercase tracking-[0.32em] text-stone-500">Status</p>
                    <p className={`mt-1 text-base font-semibold ${isActive ? "text-lime-300" : "text-stone-400"}`}>
                      {isActive ? "Active" : "Inactive"}
                    </p>
                  </div>
                  <div className="rounded-[1rem] border border-white/8 bg-white/[0.03] px-4 py-3">
                    <p className="text-xs font-semibold uppercase tracking-[0.32em] text-stone-500">Token expires</p>
              ...
              <div className="flex flex-col gap-3">
                {actions.map((action, i) => {
                  const isActionFocused = hasFocus && i === actionIndex && !action.disabled;
                  return (
                    <div
                      key={action.label}
                      onClick={() => {
                        ...
                        onSetActionIndex(i);
                        if (i === 0) onSwitch(account.id);
                        else onRemove(account.id);
                      }}
                      onMouseEnter={() => { if (!action.disabled) onSetActionIndex(i); }}
                    ...
                    >
                      <div>
                        <p className={`text-xl font-semibold ${
                          isActionFocused && !action.danger ? "text-slate-950" : "text-white"
                        }`}>{action.label}</p>
                        <p className={`mt-1 text-sm ${
                          isActionFocused
                            ? action.danger ? "text-red-300/70" : "text-slate-900/65"
                            : "text-stone-500"
          ...
          <div className="simple-panel flex flex-1 flex-col items-center justify-center gap-6 rounded-[2rem] px-14 py-12 text-center">
            ...
            <div>
              <h2 className="font-display text-4xl tracking-[-0.05em] text-white">Add an Account</h2>
            ...
            >
              ...
              <p className={`mt-1 text-sm ${
                hasFocus && focusedIndex === items.length - 1 ? "text-slate-900/65" : "text-stone-400"
              }`}>
