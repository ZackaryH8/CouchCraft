...
import { UI_SOUND_VOLUME_STEP, RAM_OPTIONS } from "../constants";
...
type SettingsOverlay = "none" | "toggle" | "slider" | "select";
...
export function useSettingsPage({
    ...
    const [settingsFocus, setSettingsFocus] =
        useState<SettingsFocus>("sections");
    ...
    const [draftSelectIndex, setDraftSelectIndex] = useState(0);
    const initialVolumeRef = useRef(uiSoundVolume);
    ...
    const openOverlay = useCallback(
        (item: SettingsItem) => {
            if (item.editType === "toggle") {
                ...
                if (item.label === "Primary Layout")
                    initialTrue = controllerLayout === "xbox";
                else if (item.label === "Motion") initialTrue = !motionReduced;
                else if (item.label === "Vibration")
                    initialTrue = vibrationEnabled;
                else if (item.label === "Fullscreen")
                    initialTrue = isFullscreen;
                else initialTrue = uiSoundsEnabled;
                setDraftToggle(initialTrue);
                setOverlay("toggle");
            } else if (item.editType === "slider") {
                initialVolumeRef.current = uiSoundVolume;
                setDraftVolume(uiSoundVolume);
                setOverlay("slider");
            } else if (item.editType === "select" && item.selectOptions) {
                const idx = item.selectOptions.indexOf(item.value);
                setDraftSelectIndex(idx >= 0 ? idx : 0);
                setOverlay("select");
            }
        },
        [
    ...
    const handleInput = useCallback(
        (input: GamepadInput) => {
                ...
                switch (input) {
                    ...
                    case "A":
                        if (selectedItem?.label === "Primary Layout") {
                            if (draftToggle !== (controllerLayout === "xbox"))
                                toggleControllerLayout();
                        } else if (selectedItem?.label === "Motion") {
                            if (draftToggle !== !motionReduced)
                                toggleMotionReduced();
                        } else if (selectedItem?.label === "Vibration") {
                            if (draftToggle !== vibrationEnabled)
                                toggleVibration();
                        } else if (selectedItem?.label === "Fullscreen") {
                            if (draftToggle !== isFullscreen)
                                toggleFullscreen();
                        } else {
                ...
                switch (input) {
                    case "LEFT": {
                        const next = parseFloat(
                            Math.max(
                                0,
                                draftVolume - UI_SOUND_VOLUME_STEP,
                            ).toFixed(2),
                        );
                        setDraftVolume(next);
                        setVolume(next);
                    ...
                    case "B":
                        ...
                        setDraftVolume(initialVolumeRef.current);
                        setOverlay("none");
                        break;
                }
                return;
            ...
            // ── overlay: select ──────────────────────────────────────────────────
            if (overlay === "select") {
                const optCount = selectedItem?.selectOptions?.length ?? 0;
                switch (input) {
                    case "LEFT":
                        setDraftSelectIndex((i) => Math.max(i - 1, 0));
                        break;
                    case "RIGHT":
                        setDraftSelectIndex((i) =>
                            Math.min(i + 1, optCount - 1),
                        );
                        break;
                    case "A":
                        if (selectedItem?.label === "Default RAM") {
                            setDefaultRamMb(RAM_OPTIONS[draftSelectIndex]);
                        }
                        setOverlay("none");
                        break;
                    case "B":
...
export function SettingsPage({
    ...
    return (
        <section className="relative grid min-h-0 flex-1 grid-cols-[18rem_1fr] gap-6">
                ...
                <div className="absolute inset-0 z-10 flex items-center justify-center rounded-[2rem] bg-black/70 backdrop-blur-sm">
                    <div className="simple-panel w-[34rem] rounded-[2rem] px-10 py-9">
                        ...
                        <div className="mt-8 flex gap-4">
                            {(
                                selectedItem?.toggleLabels ??
                                (["On", "Off"] as [string, string])
                            ).map((label, i) => {
                                const isChosen =
                                    i === 0 ? draftToggle : !draftToggle;
                                return (
                                    <div
                                        key={label}
            ...
            {/* Select overlay */}
            {overlay === "select" && selectedItem?.selectOptions && (
                <div className="absolute inset-0 z-10 flex items-center justify-center rounded-[2rem] bg-black/70 backdrop-blur-sm">
                    <div className="simple-panel w-[42rem] rounded-[2rem] px-10 py-9">
                        <p className="text-xs font-semibold uppercase tracking-[0.35em] text-stone-500">
                            Editing Setting
                        </p>
                        <h2 className="mt-3 font-display text-4xl tracking-[-0.05em] text-white">
                        ...
                        <div className="mt-8 grid grid-cols-3 gap-3">
                            {selectedItem.selectOptions.map((opt, i) => {
                                const isChosen = i === draftSelectIndex;
                                return (
                                    <div
                                        key={opt}
                                        className={`rounded-[1.25rem] border px-4 py-4 text-center transition duration-200 ${
                                            isChosen
                                                ? "border-lime-300/60 bg-lime-300 text-slate-950"
                                                : "border-white/8 bg-white/[0.03] text-stone-400"
                                        }`}
