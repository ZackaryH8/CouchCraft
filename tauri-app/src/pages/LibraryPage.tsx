...
const LIBRARY_ACTIONS = [
    {
        ...
        label: "Open",
        description:
            "Manage mods, settings, worlds, and logs for this instance",
    },
    {
        id: "launch" as const,
        label: "Launch",
    ...
    const handleInput = useCallback(
        (input: GamepadInput) => {
            ...
            switch (input) {
                ...
                case "DOWN":
                    setActionIndex((i) =>
                        Math.min(i + 1, LIBRARY_ACTIONS.length - 1),
                    );
                    break;
                case "LEFT":
                case "B":
            ...
            {overlay === "color-picker" && (
                <div className="absolute inset-0 z-10 flex items-center justify-center rounded-[2rem] bg-black/70 backdrop-blur-sm">
                    <div className="simple-panel w-[28rem] rounded-[2rem] px-10 py-9">
                        ...
                        <div className="mt-6 grid grid-cols-4 gap-4 p-2 -m-2">
                            {INSTANCE_COLORS.map((color, i) => (
                                <button
                                    key={color}
                                    type="button"
                                    className={`relative aspect-square rounded-[1.25rem] border-2 transition duration-200 ${
                                        i === colorIndex
                                            ? "z-10 scale-110 border-lime-300"
                                            : "border-transparent"
                                    }`}
                ...
                {instances.map((instance, i) => {
                    ...
                    return (
                        ...
                        >
                            ...
                            {isSelected && (
                                <div className="h-2 w-2 shrink-0 rounded-full bg-lime-300" />
                            )}
                        </div>
                    );
                })}
            ...
            {selectedInstance ? (
                            ...
                            {[
                                {
                                    ...
                                    value: formatModCount(
                                        selectedInstance.modCount,
                                    ),
                                },
                                {
                                    label: "Playtime",
                                    value: formatPlaytime(
                                        selectedInstance.playTimeSecs,
                                    ),
                                },
                                {
                                    label: "Last Played",
                                ...
                                {
                                    label: "Java",
                                    value: `Java ${selectedInstance.javaVersion}`,
                                },
                            ].map((item) => (
                                <div
                                    key={item.label}
                ...
                <div className="simple-panel flex flex-col items-center justify-center rounded-[1.75rem] px-10 py-12 text-center">
                    <div className="flex h-16 w-16 items-center justify-center rounded-full border-2 border-lime-300/40 bg-lime-300/10">
                        <span className="text-4xl font-light text-lime-300">
                            +
                        </span>
                    </div>
                    ...
                    <p className="mt-3 text-lg leading-7 text-stone-400">
                        Set up a new Minecraft instance with your choice of
                        loader and version.
                    </p>
