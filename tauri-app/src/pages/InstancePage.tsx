...
const OVERVIEW_ACTIONS = [
    ...
    },
    {
        id: "rename",
        label: "Rename",
        description: "Change the display name of this instance",
        danger: false,
    },
    {
        id: "color",
    ...
    },
    {
        id: "ram",
        label: "Edit RAM",
        description: "Set how much memory this instance can use",
        danger: false,
    },
    {
        id: "jvm",
        label: "Edit JVM Args",
        description: "Customize Java launch arguments",
        danger: false,
    },
    {
        id: "notes",
        label: "Edit Notes",
        description: "Add a short description to this instance",
        danger: false,
    },
    {
        id: "delete",
        label: "Delete Instance",
        description: "Permanently remove this instance and all files",
        danger: true,
    },
] as const;
...
export function useInstancePage({
    ...
    const modsTab = useContentTab({
        ...contentTabArgs,
        category: "mod",
        isActive: activeTab === 1,
    });
    const resourcepackTab = useContentTab({
        ...contentTabArgs,
        category: "resourcepack",
        isActive: activeTab === 2,
    });
    const datapackTab = useContentTab({
        ...contentTabArgs,
        category: "datapack",
        isActive: activeTab === 3,
    });
    const shaderTab = useContentTab({
        ...contentTabArgs,
        category: "shader",
        isActive: activeTab === 4,
    });

    const contentTabs: ContentTabState[] = [
        modsTab,
        resourcepackTab,
        datapackTab,
        shaderTab,
    ...
    const worldsTab = useWorldsTab({
        ...
        onLaunch: (folder) =>
            instance &&
            void launchInstance(instance, { quickplayWorld: folder }),
    });
    ...
    const serversTab = useServersTab({
        ...
        onLaunch: (ip) =>
            instance && void launchInstance(instance, { quickplayServer: ip }),
    });
    ...
    useEffect(() => {
        ...
        setOverlay("none");
        if (instance) {
            const ci = (INSTANCE_COLORS as readonly string[]).indexOf(
                instance.color,
            );
    ...
    const handleInput = useCallback(
        (input: GamepadInput) => {
            ...
            if (overlay === "color") {
                ...
                switch (input) {
                    ...
                    case "A":
                        void updateInstance({
                            ...instance,
                            color: INSTANCE_COLORS[colorIndex] as string,
                        });
                        setOverlay("none");
                        break;
                    case "B":
            ...
            if (overlay === "ram") {
                switch (input) {
                    case "UP":
                        setRamIndex((i) => Math.max(i - 1, 0));
                        break;
                    case "DOWN":
                        setRamIndex((i) =>
                            Math.min(i + 1, RAM_OPTIONS.length - 1),
                        );
                        break;
                    case "A":
                        void updateInstance({
                            ...instance,
                            ramMb: RAM_OPTIONS[ramIndex] ?? instance.ramMb,
                        });
                        setOverlay("none");
                        break;
                    case "B":
            ...
            if (activeTab === 6) {
                if (worldsTab.handleInput(input)) return;
            }
            if (activeTab === 7) {
                if (serversTab.handleInput(input)) return;
            }
            if (activeTab === 8) {
                if (logsTab.handleInput(input)) return;
            }
            ...
            switch (input) {
                ...
                case "LB":
                    setActiveTab((t) => {
                        for (let i = t - 1; i >= 0; i--) {
                            if (availableTabs.has(i)) return i;
                        }
                        return t;
                    });
                    return;
            ...
            if (activeTab === 0) {
                switch (input) {
                    ...
                    case "DOWN":
                        setActionIndex((i) =>
                            Math.min(i + 1, OVERVIEW_ACTIONS.length - 1),
                        );
                        break;
                    case "LEFT":
                        toSidebar();
                    ...
                    case "A": {
                        ...
                        if (id === "launch") {
                            ...
                            const ci = (
                                INSTANCE_COLORS as readonly string[]
                            ).indexOf(instance.color);
                            setColorIndex(ci >= 0 ? ci : 0);
                            setOverlay("color");
                        } else if (id === "ram") {
                            const ri = RAM_OPTIONS.indexOf(instance.ramMb);
                            setRamIndex(
                                ri >= 0 ? ri : RAM_OPTIONS.indexOf(4096),
                            );
                            setOverlay("ram");
                        } else if (id === "jvm") {
                            openOSK(
...
function ContentTabView({ tab, tabName, hasFocus }: ContentTabViewProps) {
    const {
        items,
        installedProjectIds,
        view,
    ...
    useEffect(() => {
        installedRefs.current[installedIndex]?.scrollIntoView({
            block: "nearest",
        });
    }, [installedIndex]);
    useEffect(() => {
        browseRefs.current[browseIndex]?.scrollIntoView({ block: "nearest" });
    }, [browseIndex]);
        ...
        <div className="relative flex min-h-0 flex-1 flex-col gap-4">
                        ...
                        {versionOverlay.isFetching ? (
                            ...
                            <div className="mt-5 space-y-3">
                                <div className="rounded-[1rem] border border-white/8 bg-[#121514] px-5 py-4">
                                    ...
                                    <p className="mt-1 font-semibold text-white">
                                        {versionOverlay.file.versionName}
                                    </p>
                                    <p className="text-sm text-stone-500">
                                        {versionOverlay.file.versionNumber}
                                    </p>
                                </div>
                                <div className="grid grid-cols-3 gap-3">
                                    ...
                                    <div className="rounded-[1rem] border border-white/8 bg-[#121514] px-4 py-3">
                                        ...
                                        <p className="mt-1 font-semibold capitalize text-white">
                                            {versionOverlay.file.versionType}
                                        </p>
                                    </div>
                                    <div className="rounded-[1rem] border border-white/8 bg-[#121514] px-4 py-3">
                                        <p className="text-xs font-semibold uppercase tracking-[0.3em] text-stone-500">
                                        ...
                                        <p className="mt-1 font-semibold capitalize text-white">
                                            {versionOverlay.file.loaders.join(
                                                ", ",
                                            ) || "Any"}
                                        </p>
                                    </div>
                            ...
                            <div className="mt-5">
                                <p className="text-stone-400">
                                    No compatible version found for this
            ...
            {deleteOverlay && (
                <ConfirmDialog
                    ...
                    confirmLabel="Delete"
                    warning={
                        deleteOverlay.fromModpack ? (
                            <div className="rounded-[1rem] border border-amber-400/30 bg-amber-950/40 px-4 py-3">
                                <p className="text-sm font-semibold text-amber-300">
                                    Part of modpack
                                </p>
                                <p className="mt-1 text-sm text-amber-200/70">
                                    This mod was installed as part of the
                                    modpack. Removing it may break things or
                                    cause crashes.
                                </p>
                            </div>
                        ) : undefined
                            ...
                            <div className="simple-panel flex w-[32rem] flex-col items-center gap-4 rounded-[2rem] px-12 py-10">
                                ...
                                <div>
                                    ...
                                    <p className="mt-2 text-base leading-7 text-stone-500">
                                        Browse Modrinth to find and install
                                        compatible {tabName.toLowerCase()}.
                                    </p>
                                </div>
                            </div>
                        </div>
                    ...
                    {isSearching ? (
                        ...
                        <div className="flex flex-1 flex-col items-center justify-center gap-3 text-center">
                            ...
                            <p className="text-sm text-stone-600">
                                Use Y to search for {tabName.toLowerCase()}.
                            </p>
                        </div>
                    ) : (
                        <div className="flex flex-col gap-2 overflow-y-auto">
...
function FilesTabView({ tab, hasFocus }: FilesTabViewProps) {
    const {
        entries,
        currentPath,
        selectedIndex,
    ...
    const breadcrumb = currentPath
        ? `.minecraft/${currentPath.replace(/\//g, " / ")}`
        : ".minecraft";
    ...
    return (
        <div className="relative flex min-h-0 flex-1 flex-col gap-3">
            {deleteOverlay && (
                <ConfirmDialog
                    title={`Delete ${deleteOverlay.isDir ? "Folder" : "File"}?`}
                    message={`"${deleteOverlay.name}" will be permanently removed.${deleteOverlay.isDir ? " All contents will be deleted." : ""}`}
                    choice={deleteChoice}
                    confirmLabel="Delete"
                    danger
                />
                    ...
                    {entries.map((entry, i) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className="min-w-0 flex-1">
                                    <p
                                        className={`truncate font-semibold ${entry.isDir ? "text-white" : "text-stone-300"}`}
                                    >
                                        {entry.name}
                                    </p>
                                    {!entry.isDir && (
...
function WorldsTabView({
    ...
    const { worlds, selectedIndex, isLoading, onSelectWorld, onLaunchWorld } =
        tab;
    const itemRefs = useRef<(HTMLDivElement | null)[]>([]);
    useEffect(() => {
        itemRefs.current[selectedIndex]?.scrollIntoView({ block: "nearest" });
    ...
    if (worlds.length === 0)
        return (
            <div className="flex flex-1 flex-col items-center justify-center gap-2 text-center">
                ...
                <p className="text-sm text-stone-600">
                    Launch the instance once to create a world.
                </p>
            </div>
        );
    return (
            ...
            {worlds.map((world, i) => {
                ...
                return (
                    <div
                        ...
                        onDoubleClick={() => onLaunchWorld(world.folder)}
    ...
    if (servers.length === 0)
        return (
            <div className="flex flex-1 flex-col items-center justify-center gap-2 text-center">
                ...
                <p className="text-sm text-stone-600">
                    Add servers in-game and they'll appear here.
                </p>
            </div>
        );
    return (
            ...
            {servers.map((server, i) => {
                ...
                return (
                    <div
                        ...
                        onDoubleClick={() => onLaunchServer(server.ip)}
                        className={`flex cursor-default items-center gap-4 rounded-[1.25rem] border px-5 py-4 transition duration-200 ${isSelected ? "border-lime-300/60 bg-[#171c1a]" : "border-white/8 bg-[#121514]"}`}
                    >
                        {server.icon ? (
                            <img
            ...
            {showCrash && crashReport && (
                <div className="absolute inset-0 z-10 flex items-center justify-center rounded-[2rem] bg-black/80 backdrop-blur-sm">
                    <div className="simple-panel flex h-[70%] w-[90%] flex-col rounded-[2rem] px-8 py-7">
                        ...
                        <pre
                            ref={tab.crashScrollRef}
                            className="mt-4 flex-1 overflow-y-auto font-mono text-xs leading-5 text-stone-400 whitespace-pre-wrap"
                        >
                            {crashReport}
                        </pre>
                    </div>
            ...
            {isLoading ? (
                ...
                <div className="flex flex-1 flex-col items-center justify-center gap-2">
                    <p className="text-stone-500">
                        No log file yet. Launch the instance to generate one.
                    </p>
                </div>
            ) : (
                <div
                    ref={tab.scrollRef}
                    className="flex-1 overflow-y-auto rounded-[1.25rem] border border-white/8 bg-black/40 px-5 py-4"
                >
                    {lines.map((line, i) => (
                        <p
                            key={i}
                            className={`font-mono text-xs leading-5 ${lineColor(line)}`}
                        >
                            {line}
                        </p>
                    ))}
    ...
    return (
        ...
        >
                    ...
                    <div className="simple-panel w-[24rem] rounded-[2rem] px-10 py-9">
                        ...
                        <div className="mt-5 flex flex-col gap-2">
                            {RAM_OPTIONS.map((mb, i) => (
                                ...
                                >
                                    <span
                                        className={`font-semibold ${i === ramIndex ? "text-slate-950" : "text-white"}`}
                                    >
                                        {formatRam(mb)}
                                    </span>
                                    {i === ramIndex && (
                ...
                {TABS.map((tab, i) => {
                    ...
                    return (
                        ...
                        >
                            ...
                            {i >= 1 &&
                                i <= 4 &&
                                (contentTabs[i - 1]?.items.length ?? 0) > 0 && (
                                    <span
                                        className={`ml-2 rounded-full px-1.5 text-xs font-bold ${isActive && hasFocus ? "bg-slate-950/20 text-slate-950" : "bg-white/10 text-stone-400"}`}
                                    >
            ...
            {activeTab === 0 ? (
                    ...
                    <div className="flex flex-col gap-4 overflow-y-auto">
                        <div className="relative overflow-hidden rounded-[1.75rem] border border-white/8 p-6">
                            ...
                            <div className="relative flex items-center gap-6">
                                ...
                                <div>
                                    <h2 className="font-display text-5xl tracking-[-0.06em] text-white">
                                        {instance.name}
                                    </h2>
                            ...
                            <div className="relative mt-5 grid grid-cols-4 gap-3">
                                {[
                                    ...
                                    {
                                        label: "RAM",
                                        value: formatRam(instance.ramMb),
                                    },
                                    {
                                        label: "Loader",
                                        value: LOADER_LABELS[
                        ...
                        {OVERVIEW_ACTIONS.map((action, i) => {
                            ...
                            return (
                                ...
                                >
                                    <div>
                                        <p
                                            className={`text-xl font-semibold ${isActive ? (action.danger ? "text-red-200" : "text-slate-950") : "text-white"}`}
                                        >
                                            {action.label}
                                        </p>
                                        <p
                                            className={`mt-0.5 text-sm ${isActive ? (action.danger ? "text-red-300/70" : "text-slate-900/65") : "text-stone-500"}`}
                                        >
                                            {action.description}
                                        </p>
                                    </div>
                                    {isActive && (
                                        <div
                                            className={`h-2.5 w-2.5 shrink-0 rounded-full ${action.danger ? "bg-red-400" : "bg-slate-950"}`}
                                        />
                                    )}
                                </div>
                            );
                ...
                /* Files tab ─────────────────────────────────────────────────────── */
                <FilesTabView tab={filesTab} hasFocus={hasFocus} />
            ) : activeTab === 6 ? (
                /* Worlds tab ────────────────────────────────────────────────────── */
