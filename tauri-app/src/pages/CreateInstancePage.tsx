...
const CUSTOM_STEPS: CreateStep[] = ["loader", "version", "loader_version", "color", "confirm"];
...
function nextCustomStep(step: CreateStep, loader: string): CreateStep {
  ...
  if (next === "loader_version" && loader === "vanilla") return CUSTOM_STEPS[idx + 2] ?? "confirm";
  return next;
}
...
function prevCustomStep(step: CreateStep, loader: string): CreateStep | null {
  ...
  if (prev === "loader_version" && loader === "vanilla") return CUSTOM_STEPS[idx - 2] ?? null;
  return prev ?? null;
}
...
export function useCreatePage({
  ...
  const [selectedModpack, setSelectedModpack] = useState<ModrinthHit | null>(null);
  const [modpackVersions, setModpackVersions] = useState<ModpackVersionInfo[]>([]);
  const [modpackVersionsLoading, setModpackVersionsLoading] = useState(false);
  const [isInstalling, setIsInstalling] = useState(false);
  const [installProgress, setInstallProgress] = useState<PrepareProgress | null>(null);
  ...
  const filteredLoaderVersions = useMemo(
    () => (showOnlyStable ? loaderVersions.filter((v) => v.stable) : loaderVersions).map((v) => v.version),
  ...
  const effectiveSteps = useMemo<CreateStep[]>(() => {
    ...
    const custom =
      draftLoader === "vanilla"
        ...
        : ["source", "loader", "version", "loader_version", "color", "confirm"];
  ...
  const selectItem = useCallback(
    (i: number) => {
      ...
      if (createStep === "source") {
        ...
        setCreateItemIndex(0);
        void invoke<ModpackVersionInfo[]>("list_modpack_versions", { projectId: pack.projectId })
          .then((versions) => {
    ...
    <section className="relative grid min-h-0 flex-1 grid-cols-[minmax(0,0.95fr)_minmax(20rem,0.7fr)] gap-6">
      ...
      {isInstalling && (
        <div className="absolute inset-0 z-20 flex flex-col items-center justify-center gap-6 rounded-[1.75rem] bg-[#0d0f0e]/90 backdrop-blur-sm">
          ...
          {installProgress && (
            <>
              <p className="text-xl font-semibold text-white">{installProgress.message}</p>
              <div className="w-96 overflow-hidden rounded-full bg-white/10">
                <div
                  ...
                  style={{ width: `${installProgress.total > 0 ? (installProgress.done / installProgress.total) * 100 : 0}%` }}
                />
              </div>
              <p className="text-sm text-stone-500 uppercase tracking-widest">{installProgress.stage}</p>
            </>
          )}
        </div>
      )}
      ...
      {installError && (
        <div className="absolute inset-0 z-20 flex flex-col items-center justify-center gap-6 rounded-[1.75rem] bg-[#0d0f0e]/90 backdrop-blur-sm">
          <p className="text-xl font-semibold text-red-400">Installation failed</p>
          <p className="max-w-xl text-center text-sm text-stone-400">{installError}</p>
          <button
            type="button"
      ...
      <div className="flex min-h-0 flex-col gap-5">
          ...
          <div className="flex flex-1 flex-col gap-3">
            {[
              ...
              <article
                ...
                style={{ "--accent": opt.accent } as CSSProperties}
              ...
              >
                ...
                <div className="relative">
                  <div className="h-10 w-10 rounded-[0.75rem]" style={{ background: opt.accent }} />
                  <h3 className="mt-4 font-display text-3xl tracking-[-0.04em] text-white">{opt.label}</h3>
                  <p className="mt-2 text-base leading-6 text-stone-400">{opt.sub}</p>
                </div>
                {isFocused(i) && <div className="absolute right-5 top-5 h-3 w-3 rounded-full bg-lime-300" />}
              </article>
            ))}
          </div>
        )}
            ...
            {LOADERS.map((loader, i) => (
              ...
              >
                ...
                <div className="relative">
                  ...
                  <h3 className="mt-4 font-display text-3xl tracking-[-0.04em] text-white">{loader.label}</h3>
        ...
        {step === "version" && (
          <div className="flex min-h-0 flex-col gap-3">
            <div className="flex items-center gap-3">
              <div className={`rounded-full border px-4 py-1.5 text-sm font-semibold transition duration-200 ${!showSnapshots ? "border-lime-300/60 bg-lime-300/10 text-lime-300" : "border-white/10 bg-white/5 text-stone-400"}`}>
                Releases
              </div>
              <div className={`rounded-full border px-4 py-1.5 text-sm font-semibold transition duration-200 ${showSnapshots ? "border-lime-300/60 bg-lime-300/10 text-lime-300" : "border-white/10 bg-white/5 text-stone-400"}`}>
                + Snapshots
        ...
        {step === "loader_version" && (
          <div className="flex min-h-0 flex-col gap-3">
            <div className="flex items-center gap-3">
              <div className={`rounded-full border px-4 py-1.5 text-sm font-semibold transition duration-200 ${showOnlyStable ? "border-lime-300/60 bg-lime-300/10 text-lime-300" : "border-white/10 bg-white/5 text-stone-400"}`}>
                Stable only
              </div>
              <div className={`rounded-full border px-4 py-1.5 text-sm font-semibold transition duration-200 ${!showOnlyStable ? "border-lime-300/60 bg-lime-300/10 text-lime-300" : "border-white/10 bg-white/5 text-stone-400"}`}>
                All versions
            ...
            <div className="grid grid-cols-4 gap-4">
              {INSTANCE_COLORS.map((color, i) => (
                <button
                  ...
                  onClick={() => onSelect(i)}
                  onMouseEnter={() => onMoveFocus(i)}
                  className={`aspect-square rounded-[1.5rem] border-2 transition duration-200 ${isFocused(i) ? "scale-110 border-lime-300" : "border-transparent"}`}
                  style={{ background: color }}
                />
              ))}
            </div>
            <div className="rounded-[1.25rem] border border-white/8 bg-white/[0.03] px-5 py-4">
              <p className="text-xs font-semibold uppercase tracking-[0.32em] text-stone-500">Instance Name</p>
              <p className="mt-2 text-xl font-semibold text-white">{instanceName}</p>
              <p className="mt-1 text-sm text-stone-500">Press A to confirm color. You'll name the instance next.</p>
            </div>
          </div>
        )}
        ...
        {step === "confirm" && (
          <div className="flex flex-col gap-4">
            <div className="rounded-[1.75rem] border border-white/8 bg-[#121514] p-6">
              <div className="flex items-center gap-5">
                <div
                  ...
                  style={{ background: `linear-gradient(145deg, rgba(255,255,255,0.12), rgba(0,0,0,0.16)), ${draftColor}` }}
                />
                <div>
                  <h3 className="font-display text-3xl tracking-[-0.04em] text-white">{instanceName}</h3>
                  <p className="mt-1 text-stone-400">{currentLoader?.label} · Minecraft {draftVersion}</p>
                </div>
              </div>
              <div className="mt-5 grid grid-cols-4 gap-3">
                {[
                  ...
                  ...(draftLoader !== "vanilla" ? [{ label: "Loader Ver.", value: draftLoaderVersion || "Latest" }] : []),
                  ...
                  <div key={item.label} className="rounded-[1rem] border border-white/8 bg-white/[0.03] px-4 py-3">
                    <p className="text-xs font-semibold uppercase tracking-[0.32em] text-stone-500">{item.label}</p>
            ...
            >
              <p className={`text-xs font-semibold uppercase tracking-[0.34em] ${isFocused(0) ? "text-slate-900/60" : "text-stone-500"}`}>
                Ready to create
              </p>
                  ...
                  >
                    <div className="flex items-start gap-4">
                      ...
                      <div className="min-w-0">
                        <h3 className="font-display text-xl leading-none tracking-[-0.04em] text-white">{pack.title}</h3>
                        <p className="mt-1.5 line-clamp-2 text-sm leading-5 text-stone-400">{pack.description}</p>
                      </div>
                    </div>
                    <div className="mt-3 flex gap-3 text-xs text-stone-500">
                      <span>{pack.downloads.toLocaleString()} downloads</span>
                      {pack.versions[0] && <span>· {pack.versions[0]}</span>}
                    </div>
                    {isFocused(i) && <div className="absolute right-4 top-4 h-2.5 w-2.5 rounded-full bg-lime-300" />}
                  </article>
                ))}
              </div>
            )}
            ...
            {selectedModpack && (
              <div className="flex items-center gap-3 rounded-[1rem] border border-white/8 bg-white/[0.03] px-4 py-3">
                {selectedModpack.iconUrl && (
                  <img src={selectedModpack.iconUrl} alt="" className="h-10 w-10 shrink-0 rounded-[0.75rem] object-cover" />
                )}
                <div>
                  <p className="font-semibold text-white">{selectedModpack.title}</p>
                  <p className="text-xs text-stone-500">{selectedModpack.description}</p>
                </div>
              </div>
            )}
            {modpackVersionsLoading ? (
              ...
              <div className="flex flex-1 flex-col items-center justify-center gap-4 text-center">
                <p className="text-lg text-stone-400">No versions available.</p>
                <p className="text-sm text-stone-600">Press B to go back.</p>
              </div>
            ) : (
                  ...
                  >
                    <div>
                      <div className="flex items-center gap-2.5">
                        <p className="text-lg font-semibold text-white">{ver.versionName}</p>
                        {i === 0 && (
      ...
      <aside className="simple-panel flex flex-col rounded-[1.75rem] px-6 py-6">
          ...
          <div className="mt-4">
            {selectedModpack ? (
              <>
                ...
                <p className="mt-2 text-sm leading-5 text-stone-400">{selectedModpack.description}</p>
                <div className="mt-4 space-y-2 text-sm text-stone-500">
                  <p>{selectedModpack.downloads.toLocaleString()} downloads</p>
                  <p>{selectedModpack.follows.toLocaleString()} follows</p>
                </div>
              </>
            ) : (
              <p className="mt-4 text-sm text-stone-600">Select a modpack to see details.</p>
            )}
          </div>
        ) : (
          <>
            <div className="mt-4 flex items-center gap-4">
              <div
                ...
                style={{ background: `linear-gradient(145deg, rgba(255,255,255,0.12), rgba(0,0,0,0.16)), ${draftColor}` }}
              />
              <h3 className="font-display text-3xl leading-none tracking-[-0.05em] text-white">{instanceName}</h3>
            </div>
            <div className="mt-6 space-y-3">
              {[
                ...
                <div key={item.label} className="rounded-[1.25rem] border border-white/8 bg-white/[0.03] px-5 py-4">
                  <p className="text-xs font-semibold uppercase tracking-[0.32em] text-stone-500">{item.label}</p>
                  {item.isColor ? (
        ...
        <div className="mt-auto rounded-[1.25rem] border border-lime-300/20 bg-lime-300/8 px-5 py-4">
          <p className="text-xs font-semibold uppercase tracking-[0.32em] text-lime-300/70">Navigation</p>
