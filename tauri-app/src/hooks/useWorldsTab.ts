...
export function useWorldsTab({
    instanceId,
    isActive,
    onLaunch,
}: UseWorldsTabOptions): WorldsTabState {
    ...
    const onLaunchWorld = useCallback(
        (folder: string) => {
            onLaunch(folder);
        },
        [onLaunch],
        ...
        (input: GamepadInput): boolean => {
            switch (input) {
                case "UP":
                    ...
                    return true;
                case "DOWN":
                    setSelected((i) => Math.min(i + 1, worlds.length - 1));
                    return true;
                case "A":
                    if (worlds[selectedIndex])
                        onLaunchWorld(worlds[selectedIndex].folder);
                    return true;
                case "B":
                    return false;
            }
