    ...
    const handleInput = useCallback(
        (input: GamepadInput) => {
            ...
            if (input === "Y") {
                flash("space");
                setValue(
                    (v) => v.slice(0, cursorPos) + " " + v.slice(cursorPos),
                );
                setCursorPos((p) => p + 1);
                return;
            }
            if (input === "X") {
                ...
                if (cursorPos > 0) {
                    setValue(
                        (v) => v.slice(0, cursorPos - 1) + v.slice(cursorPos),
                    );
                    setCursorPos((p) => p - 1);
                }
                return;
            ...
            if (section === "main") {
                switch (input) {
                    ...
                    case "DOWN":
                        if (row < MAIN_ROWS.length - 1) {
                            ...
                            const nc =
                                nr === 4
                                    ? 0
                                    : Math.min(col, MAIN_ROWS[nr].length - 1);
                            setFocus((f) => ({ ...f, row: nr, col: nc }));
                        }
                        break;
                    case "LEFT":
                        if (row === 4) {
                            ...
                            setFocus((f) => ({
                                ...f,
                                col: MAIN_ROWS[0].length - 1,
                            }));
                        } else {
                            setFocus({
                                section: "left",
                                sideIdx: mainRowToLeftSideIdx(row),
                                row,
                                col: 0,
                            });
                        }
                    ...
                    case "RIGHT":
                        if (row === 4) {
                            ...
                            setFocus({
                                section: "right",
                                sideIdx: mainRowToRightSideIdx(row),
                                row,
                                col: 0,
                            });
                        }
                ...
                switch (input) {
                    case "UP":
                        if (sideIdx > 0)
                            setFocus((f) => ({ ...f, sideIdx: sideIdx - 1 }));
                        break;
                    case "DOWN":
                        if (sideIdx < LEFT_SIDEBAR.length - 1)
                            setFocus((f) => ({ ...f, sideIdx: sideIdx + 1 }));
                        break;
                    case "RIGHT":
                        setFocus({
                            section: "main",
                            sideIdx: 0,
                            row: LEFT_SIDEBAR[sideIdx].mainRowStart,
                            col: 0,
                        });
                        break;
                    case "A":
                ...
                switch (input) {
                    case "UP":
                        ...
                        break;
                    case "DOWN":
                        if (sideIdx < RIGHT_SIDEBAR.length - 1)
                            setFocus((f) => ({ ...f, sideIdx: sideIdx + 1 }));
                        break;
                    case "LEFT": {
                        const mainRow = RIGHT_SIDEBAR[sideIdx].mainRowStart;
                        const lastCol =
                            mainRow === 4 ? 0 : MAIN_ROWS[mainRow].length - 1;
                        setFocus({
                            section: "main",
                            sideIdx: 0,
                            row: mainRow,
