...
export function useNavStack(initial: NavFrame = { id: "home" }): NavStack {
  ...
  const push = useCallback((frame: NavFrame) => setStack((p) => [...p, frame]), []);
  const pop = useCallback(() => setStack((p) => (p.length > 1 ? p.slice(0, -1) : p)), []);
  const replace = useCallback((frame: NavFrame) => setStack((p) => [...p.slice(0, -1), frame]), []);
