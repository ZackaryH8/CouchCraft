...
export function useSettings() {
    ...
    const [controllerLayout, setControllerLayout] =
        useState<ControllerType>("xbox");
    const [isFullscreen, setIsFullscreen] = useState(false);
    const [motionReduced, setMotionReduced] = useState(false);
    const [vibrationEnabled, setVibrationEnabled] = useState(true);
    const [defaultRamMb, setDefaultRamMbState] = useState(2048);
    ...
    useEffect(() => {
        const applyValues = (
            ...
            if (volume !== null) {
                ...
                if (Number.isFinite(parsed))
                    setUiSoundVolume(clampVolume(parsed));
            }
            ...
            if (ram !== null) {
                ...
                if ((RAM_OPTIONS as readonly number[]).includes(parsed))
                    setDefaultRamMbState(parsed);
            }
        };
        ...
        if (isTauri()) {
            Promise.all([
                ...
                getSetting(DEFAULT_RAM_STORAGE_KEY),
            ])
                .then(
                    ([
                        sounds,
                ...
                .catch(() =>
                    applyValues(
                        ...
                        readLocalStorage(DEFAULT_RAM_STORAGE_KEY),
                    ),
                );
        } else {
            applyValues(
    ...
    const setDefaultRamMb = useCallback(
        (mb: number) => {
            setDefaultRamMbState(mb);
            persist(DEFAULT_RAM_STORAGE_KEY, String(mb));
        },
        [persist],
    );
