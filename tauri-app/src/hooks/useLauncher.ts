...
export function useLauncher({
    ...
    useEffect(() => {
        onGameExitedRef.current = onGameExited;
    }, [onGameExited]);
    ...
    const launchInstance = useCallback(
        async (
            ...
            setState((s) => ({
                ...s,
                isLaunching: true,
                launchingName: instance.name,
                progress: null,
                launchError: null,
            }));

            const unlisten = await listen<PrepareProgress>(
                "prepare-progress",
                (event) => {
            ...
            try {
                ...
                if (
                    account &&
                    account.expiresAt - Math.floor(Date.now() / 1000) < 300
                ) {
                    try {
                        const raw = await invoke<{
                            ...
                            refreshToken: account.msRefreshToken,
                        });
                        account = {
                            ...account,
