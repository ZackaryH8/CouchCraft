...
export function useServersTab({
    instanceId,
    isActive,
    onLaunch,
}: UseServersTabOptions): ServersTabState {
    ...
    const onLaunchServer = useCallback(
        (ip: string) => {
            onLaunch(ip);
        },
        [onLaunch],
            ...
            switch (input) {
                ...
                case "A":
                    if (servers[selectedIndex])
                        onLaunchServer(servers[selectedIndex].ip);
                    return true;
                case "B":
                    return false;
            }
