...
function rawToAccount(
    raw: RawAccount,
    existingId?: string,
    existingAddedAt?: number,
): McAccount {
    return {
...
export function useAuth() {
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            const [accs, activeId] = await Promise.all([
                loadAccounts(),
                getActiveAccountId(),
            ]);
            ...
            const refreshed = await Promise.all(
                accs.map(async (acc) => {
                    ...
                    try {
                        const raw = await invoke<RawAccount>(
                            "refresh_mc_auth",
                            { refreshToken: acc.msRefreshToken },
                        );
    ...
    useEffect(() => {
        ...
        const poll = async () => {
            try {
                const result = await invoke<RawAccount | null>(
                    "poll_device_code",
                    {
