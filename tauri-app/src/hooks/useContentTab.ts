...
export function useContentTab({
    ...
    const [versionOverlay, setVersionOverlay] = useState<VersionOverlay | null>(
        null,
    );
    const [deleteOverlay, setDeleteOverlay] = useState<ContentItem | null>(
        null,
    );
    ...
    const selectResult = useCallback(
        async (hit: ModrinthHit) => {
            ...
            try {
                const file = await invoke<ModrinthVersionFile | null>(
    ...
    const confirmDelete = useCallback(async (item: ContentItem) => {
        try {
        ...
        (input: GamepadInput): boolean => {
            ...
            if (deleteOverlay) {
                switch (input) {
                    ...
                    case "A":
                        if (deleteChoice === 1)
                            void confirmDelete(deleteOverlay);
            ...
            if (view === "browse") {
                switch (input) {
                    ...
                    case "A": {
                        ...
                        if (hit && !installedProjectIds.has(hit.projectId))
