...
pub async fn prepare_instance(app: tauri::AppHandle, config: PrepareConfig) -> Result<(), String> {
    ...
    let emit = |stage: &str, msg: &str, done: u32, total: u32| {
        let _ = app.emit("prepare-progress", PrepareProgress {
            stage: stage.to_string(), message: msg.to_string(), done, total,
        });
    };

    // 1. Fetch version JSON (vanilla + loader layer)
    emit("versions", "Fetching version manifest…", 0, 1);
    let version = resolve_version(&client, &base, &config.mc_version, &config.loader_type, &config.loader_version).await?;
    ...
    emit("java", &format!("Checking Java {}…", min_java), 0, 1);
    {
        ...
        ensure_java(&client, &base, &mc, move |done, total| {
            let _ = app2.emit("prepare-progress", PrepareProgress {
                stage: "java".into(),
                message: format!("Downloading Java {}… ({}%)", min_java, if total > 0 { done as u64 * 100 / total as u64 } else { 0 }),
                done,
                total,
            });
        }).await?;
    ...
    if let Some(dl) = &version.downloads {
        ...
        let jar_path = base.join("versions").join(&config.mc_version).join(format!("{}.jar", config.mc_version));
        download_checked(&client, &dl.client.url, &jar_path, dl.client.sha1.as_deref()).await?;
    ...
    for (i, (path, url, sha1)) in libs.into_iter().enumerate() {
        ...
        handles.push(tokio::spawn(async move {
            ...
            let _ = app2.emit("prepare-progress", PrepareProgress {
                ...
                message: format!("Library {}", dest.file_name().unwrap_or_default().to_string_lossy()),
                done: (i + 1) as u32,
                total: total_libs,
            });
            result
    ...
    if let Some(idx_ref) = &version.asset_index {
        let idx_path = base.join("assets").join("indexes").join(format!("{}.json", idx_ref.id));
        ...
        for (i, obj) in objects.into_iter().enumerate() {
            ...
            let dest = base.join("assets").join("objects").join(prefix).join(&obj.hash);
...
pub async fn launch_game(app: tauri::AppHandle, config: LaunchConfig) -> Result<(), String> {
    ...
    let game_dir = base.join("instances").join(&config.instance_id).join(".minecraft");
    std::fs::create_dir_all(&game_dir).map_err(|e| e.to_string())?;
    let assets_dir = base.join("assets");
    let asset_id = version.asset_index.as_ref().map(|a| a.id.as_str()).unwrap_or("legacy");

    // Build classpath
    let mut cp_entries: Vec<PathBuf> = Vec::new();
    let mut natives_dirs: Vec<PathBuf> = Vec::new();
    let natives_dir = base.join("instances").join(&config.instance_id).join("natives");
    std::fs::create_dir_all(&natives_dir).map_err(|e| e.to_string())?;

    for lib in &version.libraries {
        if !should_include(lib) { continue; }
    ...
    // Client JAR
    let client_jar = base.join("versions").join(&config.mc_version).join(format!("{}.jar", config.mc_version));
    cp_entries.push(client_jar);

    let classpath = cp_entries.iter()
    ...
    tokens.insert("game_directory".into(), game_dir.to_string_lossy().into_owned());
    tokens.insert("assets_root".into(), assets_dir.to_string_lossy().into_owned());
...
async fn resolve_version(
    ...
    match loader_type {
        "fabric" => {
            ...
            let overlay = client.get(&url).send().await.map_err(|e| e.to_string())?
                .json::<VersionJson>().await.map_err(|e| e.to_string())?;
...
async fn fetch_vanilla_version(client: &Client, base: &Path, mc_version: &str) -> Result<VersionJson, String> {
    let json_path = base.join("versions").join(mc_version).join(format!("{}.json", mc_version));
...
fn dedup_libraries(libs: Vec<Library>) -> Vec<Library> {
    ...
    libs.into_iter()
        .filter(|lib| {
            ...
            let key: String = lib.name.splitn(3, ':').take(2).collect::<Vec<_>>().join(":");
            seen.insert(key)
        })
        .collect()
}
...
fn collect_libraries(version: &VersionJson) -> Vec<(String, String, Option<String>)> {
    ...
    for lib in &version.libraries {
        ...
        if let Some(dl) = lib.downloads.as_ref().and_then(|d| d.artifact.as_ref()) {
            ...
            let path = maven_path(&lib.name);
            let url = format!("{}/{}", url_base.trim_end_matches('/'), path.replace('\\', "/"));
...
fn should_include(lib: &Library) -> bool {
    ...
    for rule in &lib.rules {
        if rule.features.is_some() {
            ...
            if rule.action == "allow" { result = false; }
            continue;
        }
...
fn maven_path(name: &str) -> String {
    ...
    if let Some(classifier) = parts.get(3) {
        format!("{}/{}/{}/{}-{}-{}.jar", group_path, artifact, version, artifact, version, classifier)
    } else {
        format!("{}/{}/{}/{}-{}.jar", group_path, artifact, version, artifact, version)
    }
}
...
fn extract_natives(jar: &Path, dest: &Path, exclude: &[String]) -> Result<(), String> {
    ...
    for i in 0..archive.len() {
        ...
        if exclude.iter().any(|ex| name.contains(ex.as_str())) { continue; }
        if !name.ends_with(".so") && !name.contains(".so.") { continue; }

        let dest_path = dest.join(&name);
        if let Some(p) = dest_path.parent() { std::fs::create_dir_all(p).map_err(|e| e.to_string())?; }
...
fn best_java_in_dir(dir: &Path, min: u32) -> Option<String> {
    ...
    let mut found: Vec<(u32, String)> = entries.flatten()
        .filter_map(|e| {
            let bin = find_java_in_dir(&e.path())?;
            let major = detect_java_major(&bin);
            if major >= min { Some((major, bin)) } else { None }
        })
        .collect();
...
fn find_java_in_dir(dir: &Path) -> Option<String> {
    ...
    if direct.exists() { return Some(direct.to_string_lossy().into_owned()); }
    None
}
...
fn find_best_java(base: &Path, mc_version: &str) -> String {
    ...
    if let Some(p) = best_java_in_dir(&base.join("jre"), min) { return p; }
    ...
    if let Ok(entries) = std::fs::read_dir("/usr/lib/jvm") {
        let mut found: Vec<(u32, String)> = entries.flatten()
            .filter_map(|e| {
                ...
                if major >= min { Some((major, bin.to_string_lossy().into_owned())) } else { None }
            })
            .collect();
        found.sort_by_key(|(v, _)| std::cmp::Reverse(*v));
        if let Some((_, p)) = found.into_iter().next() { return p; }
    }
...
async fn ensure_java(
    ...
    if best_java_in_dir(&base.join("jre"), min).is_some() { return Ok(()); }
    ...
    if std::path::Path::new("/usr/bin/java").exists()
        && detect_java_major("/usr/bin/java") >= min
    {
        return Ok(());
    }
    ...
    while let Some(chunk) = stream.next().await {
        ...
        let done = if total > 0 { (data.len() as u64).min(total) as u32 } else { data.len() as u32 };
...
fn detect_java_major(java_bin: &str) -> u32 {
    ...
    let Ok(output) = std::process::Command::new(java_bin).arg("-version").output() else {
        return 0;
    };
    let text = String::from_utf8_lossy(&output.stderr);
    // Find the quoted version string
    let Some(ver_str) = text.split('"').nth(1) else { return 0 };
        ...
        match arg {
            ...
            serde_json::Value::Object(obj) => {
                if let Some(rules) = obj.get("rules") {
                    if !eval_arg_rules(rules) { continue; }
                }
                match obj.get("value") {
