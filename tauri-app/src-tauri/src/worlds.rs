...
fn game_mode_str(mode: i32) -> &'static str {
    match mode { 1 => "Creative", 2 => "Adventure", 3 => "Spectator", _ => "Survival" }
}
...
#[tauri::command]
pub async fn list_worlds(app: tauri::AppHandle, instance_id: String) -> Result<Vec<WorldInfo>, String> {
    let saves_dir = mc_root(&app, &instance_id).join("saves");
    if !saves_dir.exists() { return Ok(vec![]); }

    let mut worlds = Vec::new();
    for entry in std::fs::read_dir(&saves_dir).map_err(|e| e.to_string())?.flatten() {
        let path = entry.path();
    ...
    worlds.sort_by(|a, b| b.last_played_ms.unwrap_or(0).cmp(&a.last_played_ms.unwrap_or(0)));
    Ok(worlds)
}
